...
function buildPrompt(control, queue, job, brief, briefFile) {
  ...
  return [
    ...
    '- the Google Stitch HTML prototypes (see CLAW/control-plane/directives/stitch-rescaffold.json) are the PRIMARY layout reference for product pages; existing geometry laws should be updated to match Stitch, not the other way around',
...
function laneExecutionRecipe(job) {
  const recipes = {
    'product-family': [
      '- CRITICAL: Read the operator directive first: `cat CLAW/control-plane/directives/stitch-rescaffold.json`.',
      '- Read the Google Stitch product page HTML prototype: `ls "/Users/zer0palab/Zer0pa Website/Zer0pa ZPE Product Pages/"` then read all .html files found there. This Stitch HTML is the authoritative layout reference for /work/[slug] pages.',
      '- Read only these repo files next: `site/src/app/work/[slug]/page.tsx`, `site/src/components/lane/LaneAuthorityPage.tsx`, `site/src/lib/data/presentation.ts`, `site/src/lib/types/lane.ts`, and `site/.cache/packets/ZPE-XR.json`.',
      '- Rebuild /work/[slug]/page.tsx to match the Stitch layout structure: section ordering, grid patterns, component hierarchy, and spacing. Wire dynamic content from loadLaneBySlug() packet cache instead of Stitch static text.',
      '- Use placeholder divs with clear data-slot attributes for visualization/widget spaces that will be filled later.',
      '- Translate Stitch Tailwind CDN classes to the project\'s PostCSS Tailwind setup. Translate Google Fonts links to next/font. Replace Stitch img src with Next.js Image component and placeholders.',
    ...
    'data-truth': [
      '- Read the XR packet from the brief\'s shared truth cache.',
      '- SCOPING RULE: Sufficiency means the packet has enough fields to populate the Stitch product page template sections. Fields may use ANY naming convention — check for equivalent data, not exact canonical names.',
      '- FIELD MAPPING (canonical → actual): name=laneTitle, tagline=tagline, status=authorityState.status, metrics=headlineMetrics, proofAssertions=provedNow, repoShape=repoShape, evidenceRoutes=verificationPath OR proofAnchors. relatedLanes is OPTIONAL for the initial C4 proof.',
      '- Verify freshness: compare the packet commitSha field against live GitHub main HEAD.',
      '- If equivalent Stitch template data is present (under any field names) and the packet is fresh, ACCEPT with commit=null and no file changes. Put your findings in the handoff JSON fields (audit_result, postflight_metrics) instead of writing a report file.',
      '- CRITICAL: Do NOT write or update report files. Do NOT reject because field names differ from idealized canonical names. The product-family agent handles field mapping at consumption time.',
      '- Do not rewrite ingestion architecture unless the XR packet is provably insufficient for Stitch template population or contradictory.',
      '- Favor the shortest possible accept path. A quick check that accepts with no file changes is more valuable than an exhaustive audit that creates cherry-pick conflicts.',
    ],
    'systems-qa': [
      '- Read the operator directive first: `cat CLAW/control-plane/directives/stitch-rescaffold.json`.',
      '- Read the Google Stitch product page HTML: `ls "/Users/zer0palab/Zer0pa Website/Zer0pa ZPE Product Pages/"` then read the .html files. Use this as the layout reference for falsification — the Stitch layout IS the design spec.',
